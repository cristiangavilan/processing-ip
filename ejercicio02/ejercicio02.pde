...
void keyPressed() {
  ...
  } else if (key == 'c') { // Si el usuario presiona la tecla c
    background(255); // Limpiamos el fondo
  } else if (key == 's') { // Si el usuario presiona la tecla s salvamos la imagen
    saveFrame("lienzo.png"); // Guardamos el lienzo
  }
}
